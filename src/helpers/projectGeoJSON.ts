...
export function projectGeoJSON(source: GeoJSON, conv = epsg28992.forward): GeoJSON {
  ...
  if (isGeometryCollection(g)) {
    ...
    g.coordinates = g.coordinates.map((y) => y.map(conv))
  } else if (isMultiPolygon(g)) {
    g.coordinates = g.coordinates.map((y) => y.map((z) => z.map(conv)))
  } else {
