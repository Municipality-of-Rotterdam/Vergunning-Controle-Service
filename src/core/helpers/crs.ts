...
import { isGeometryCollection, isPolygon, isMultiPolygon } from './isGeoJSON.js'
...
export function projectGeoJSON(source: GeoJSON, from: proj4.Converter = epsg28992): GeoJSON {
  ...
  if (isGeometryCollection(g)) {
    ...
    g.coordinates = g.coordinates.map((y) => y.map(from.inverse))
  } else if (isMultiPolygon(g)) {
    g.coordinates = g.coordinates.map((y) => y.map((z) => z.map(from.inverse)))
  } else {
